...
public class MessageChannel {
  ...
  /**
   * Removes and returns the first message posted with the given event, leaving the rest of the
   * queue untouched. Does not wait for one to arrive.
   */
  public @Nullable Message pollMessage(final String event) {
    for (Message message : queue) {
      // remove() reports whether we were the ones to take it, so two consumers looking for the
      // same event never both get it
      if (message.event.equals(event) && queue.remove(message)) {
        return message;
      }
    }
    return null;
